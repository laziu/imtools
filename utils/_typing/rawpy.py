...
class RawPy(rawpy.RawPy):
    ...
    def postprocess(self, params: "Params" = None, **kw) -> np.ndarray:
        """
...
class Params(rawpy.Params):
    """ A class that handles postprocessing parameters.

    NOTE: Typing only definitions for rawpy.Params
    """

    def __init__(self, demosaic_algorithm: DemosaicAlgorithm = None, half_size: bool = False,
                 ...
                 noise_thr: float = None, median_filter_passes: int = 0,
                 use_camera_wb: bool = False, use_auto_wb: bool = False, user_wb: list[int] = None,
                 output_color: ColorSpace = ColorSpace.sRGB, output_bps: int = 8,
                 user_flip: int = None, user_black: int = None, user_sat: int = None,
                 no_auto_bright: bool = False, auto_bright_thr: float = None, adjust_maximum_thr: float = 0.75,
                 bright: float = 1.0, highlight_mode: HighlightMode = HighlightMode.Clip,
                 exp_shift: float = None, exp_preserve_highlights: float = 0.0, no_auto_scale: bool = False,
                 gamma: tuple[float, float] = None, chromatic_aberration: tuple[float, float] = None,
