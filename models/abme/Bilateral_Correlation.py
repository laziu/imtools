...
class BilateralCorrelation(nn.Module):
    def __init__(self, md=4):
        ...
        d_u = torch.linspace(-self.md, self.md, 2 * self.md + 1) \
            .view(1, -1).repeat((2 * self.md + 1, 1)).view(self.range, 1)  # (25,1)
        d_v = torch.linspace(-self.md, self.md, 2 * self.md + 1) \
            .view(-1, 1).repeat((1, 2 * self.md + 1)).view(self.range, 1)  # (25,1)
    ...
    def warp(self, x, BM_d):
        ...
        output = nn.functional.grid_sample(x, vgrid, mode='bilinear', padding_mode='border', align_corners=True)
    ...
    def forward(self, feature1, feature2, SBM, time=0.5):
        ...
        SBM_d = SBM.view(SBM.size(0), 1, SBM.size(1), SBM.size(2), SBM.size(3)) \
            .expand(-1, self.range, -1, -1, -1) + D_vol

        BC_list = []

        for i in range(SBM.size(0)):
            bw_feature = self.warp(feature1[i].view((1,) + feature1[i].size())
                                   .expand(self.range, -1, -1, -1),  2*(-time)*SBM_d[i])  # (D**2,C,H,W)
            fw_feature = self.warp(feature2[i].view((1,) + feature2[i].size())
                                   .expand(self.range, -1, -1, -1), 2*(1-time)*SBM_d[i])  # (D**2,C,H,W)

            BC_list.append(torch.sum(torch.mul(fw_feature, bw_feature), dim=1)
                           .view(1, self.range, SBM.size(2), SBM.size(3)))
